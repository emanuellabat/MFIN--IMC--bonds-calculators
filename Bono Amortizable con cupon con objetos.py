# -*- coding: utf-8 -*-
"""
Created on Mon Aug  3 11:45:11 2020
...
class Bond_valuator:
    ...
    def cup_and_prin(self):
        ...
        Amort=self.amort
        j=0
        if self.frec_amort == 1:
            while j < Amort:
                new_list[2*j] = self.HC/self.amort
                j=j+1
        else:
            while j < Amort:
                new_list[j] = self.HC/self.amort
                j=j+1
...
def main():
    ...
    Amortizaciones=10                                                                # cantidad de amortizaciones que se quiere hacer
    frecuencia_amortizacion= 2                                                                   # frec a la que se amortiza
    cupon= 1                                                                        # es la tasa del cupon
    capitalizacion=2                                                                # la capitalización es anual=1 y semi-anual=2
    YTM= 17                                                                          # es la YTM que se desea utilizar
    
    
    F_Liquidacion=datetime.datetime.strptime('25/12/2020','%d/%m/%Y')               #Definiendo la fecha de valorización
    F_Vencimiento=datetime.datetime.strptime('09/07/2029','%d/%m/%Y')               #Definiendo la fecha de vencimiento
    F_dev_Cupon_inicial=datetime.datetime.strptime('25/12/2020','%d/%m/%Y')         #Definiendo la fecha de dev de los pagos
    F_pago_Cupon_inicial=datetime.datetime.strptime('09/07/2021','%d/%m/%Y')        #Definiendo la fecha de pago del primer cupon
    ...
    BV = Bond_valuator(
                 ...
                 cup = cupon,
                 cap = capitalizacion,
                 frec_amort = frecuencia_amortizacion,
                 ytm = YTM,
